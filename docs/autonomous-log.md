...
- 2026-06-04 — QA code-audit pass (two agents read the actual Plan-my-day + Discover source against the QA test matrices) → fixed 5 confirmed bugs. Plan my day (80c3541): "HH:MM" string-sort mis-ordered unpadded times ("9:30" after "10:00") causing mis-sequence + idempotency drift → numeric timeToMin sort; a user-LOCKED stop past the pace cap was mis-flagged as capacity overflow → excluded (pinned = intent). +3 tests. Discover (dbdf691): dietary bias was force-injected from families on top of the clearable chips so "Search without filters"/un-checking never un-filtered Eat → chips only (chips are auto-seeded from the same profile); search race/stale-wins → monotonic reqSeq guard so only the latest loadScope writes; switching cities left the old city's results+pins on screen → clear on city change. 204 tests
