      ...
      >
        ...
        <View style={s.header}>
          ...
          <View style={s.iconpill}>
            <TouchableOpacity style={s.iconbtn} onPress={() => soon('Search')}><Icon name="search" size={19} color={colors.text} /></TouchableOpacity>
            <TouchableOpacity style={s.iconbtn} onPress={() => soon('Saved trips')}><Icon name="star" size={19} color={colors.text} /></TouchableOpacity>
          </View>
        </View>
        ...
        <Section title="Popular Categories" action="See all" onAction={() => soon('Categories')} />
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.rail}>
          {CATEGORIES.map((c, i) => (
            <PressableScale key={c.label} haptic="light" style={s.cat} onPress={() => soon(c.label)}>
              <LinearGradient colors={c.g} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={StyleSheet.absoluteFill} />
        ...
        <Section title="Start your trip" />
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.rail}>
          <ActionCard emoji="✨" tint={colors.smartSoft} title="Paste a plan" sub="From ChatGPT, a blog, anywhere" onPress={() => setShowPaste(true)} />
          <ActionCard emoji="👨‍👩‍👧‍👦" tint={colors.primaryLight} title="Group trip" sub="Add families · costs auto-split" onPress={() => setShowNewTrip(true)} />
          <ActionCard emoji="🤖" tint={colors.smartSoft} title="Plan with AI" sub="Day-by-day from your vibe" onPress={() => soon('AI planning')} />
          <ActionCard emoji="📋" tint={colors.primaryLight} title="Templates" sub="Proven multi-family routes" onPress={() => soon('Templates')} />
        </ScrollView>

        {/* RAIL — Made for groups */}
        <Section title="Made for groups" action="See all" onAction={() => soon('Group itineraries')} />
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.rail}>
          {GROUP_TRIPS.map((t) => (
...
function ActionCard({ emoji, tint, title, sub, onPress }) {
  return (
...
const s = StyleSheet.create({
  ...
  h1: { fontSize: 36, fontWeight: '800', color: colors.text, letterSpacing: -0.5 },
  iconpill: { flexDirection: 'row', gap: 6, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.hairline, borderRadius: radius.full, padding: 8 },
  iconbtn: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center' },

  banner: { flexDirection: 'row', alignItems: 'center', gap: 11, marginHorizontal: spacing.lg, marginBottom: spacing.xs, padding: 13, borderRadius: radius.lg, backgroundColor: colors.smartSoft, borderWidth: 1, borderColor: colors.hairline },
  bannerEmoji: { fontSize: 22 },
  bannerTitle: { fontSize: 14, fontWeight: '800', color: colors.smartDeep },
  ...
  secTitle: { fontSize: 22, fontWeight: '800', color: colors.text, letterSpacing: -0.3 },
  ...
  act: { width: 150, height: 148, borderRadius: 20, padding: 15, justifyContent: 'space-between', borderWidth: 1, borderColor: colors.hairline, backgroundColor: colors.surface },
  actIcon: { width: 46, height: 46, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  actTitle: { fontWeight: '800', fontSize: 15, color: colors.text },
  actSub: { fontSize: 12, color: colors.muted, marginTop: 3, lineHeight: 16 },

  gtrip: { width: 230, borderRadius: 20, overflow: 'hidden', backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.hairline },
  ...
  gbody: { padding: 12 },
  gtitle: { fontWeight: '800', fontSize: 15, color: colors.text },
  chipRow: { flexDirection: 'row', gap: 6, marginTop: 6 },
  chip: { backgroundColor: colors.bg, borderWidth: 1, borderColor: colors.hairline, borderRadius: radius.full, paddingHorizontal: 8, paddingVertical: 3 },
  chipText: { fontSize: 11, fontWeight: '700', color: colors.accent },

  footnote: { paddingHorizontal: spacing.xl, paddingTop: spacing.lg, color: colors.subtle, fontSize: 12, lineHeight: 18 },
});
